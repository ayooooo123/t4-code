...
The installed Apple Silicon Mac app packages its own standalone `t4-host` and matching official OMP authority runtime. Source builds, Linux hosts, and remote hosts need the matching official OMP build with the smaller authority bridge.

T4 Code v0.1.32 was verified with OMP 17.2.4 built from [`06343fef`](https://github.com/can1357/oh-my-pi/commit/06343fef4200c4e32d18f08df5a6a8bd84dcc710), tagged [`v17.2.4`](https://github.com/can1357/oh-my-pi/tree/v17.2.4). It exposes the bounded `t4-omp-authority/1` bridge used by T4's standalone host and removes the old public OMP appserver launchers. It also includes snapshot-consistent paged authority inventories with explicit partial-state metadata, stale-lock lifecycle recovery, sparse authority session lists, atomic `xd://` mount publication, bounded transcript tail paging, session fork authority bridge support, and deterministic session file ordering.

T4 owns the client wire, generic host service, and standalone daemon in `@t4-code/host-wire`, `@t4-code/host-service`, and `@t4-code/host-daemon`. T4 Code vendors `@oh-my-pi/app-wire` 0.7.0 from integration commit [`796bb7dc`](https://github.com/lyc-aon/oh-my-pi/commit/796bb7dca45027bd4b7b94017cdf41ef214a11f2), source tree `0c195a01ba0bb98fbf4d4863aee59bf23a6e81b7`. The frozen package remains only as compatibility evidence. The released package launches `t4-host` against the strict `t4-omp-authority/1` bridge, while official OMP owns session files, locks, agent execution, and takeover decisions.
