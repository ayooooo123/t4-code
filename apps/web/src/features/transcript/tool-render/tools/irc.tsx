...
import { Badge, Badges, Note, ResultText, Row } from "../parts.tsx";
...
function parsePeers(value: unknown): IrcPeer[] {
  ...
  return out.sort(
    (a, b) => (PEER_STATUS_ORDER[a.status] ?? 9) - (PEER_STATUS_ORDER[b.status] ?? 9),
  );
}
...
function Summary({ args, result }: ToolRenderProps): ReactNode {
  ...
  const opBadge = (
    <Badge tone={result?.isError ? "err" : op === "send" ? "accent" : undefined}>{op}</Badge>
  );
  if (op === "send") {
...
function Body({ args, result }: ToolRenderProps): ReactNode {
  ...
  const structured =
    receipts.length > 0 || waited !== null || timedOut || inbox.length > 0 || peers.length > 0;
  return (
    <>
      <Badges
        items={[
      ...
      {timedOut && (
        <Note tone="warn">No reply yet — they may answer later; check inbox or wait again.</Note>
      )}
      {inbox.length > 0 && (
        <div className="tv-list">
          {inbox.map((msg, i) => (
