...
import { Badge, Badges, InvalidArg, Kv, KvGrid, Note, ResultText, Row } from "../parts.tsx";
  ...
  return (
    <>
      <KvGrid>
        <Kv k="query">
          {query !== null ? (
            <span className="tv-pattern">{query}</span>
          ) : (
            <InvalidArg what="query" />
          )}
        </Kv>
        ...
        <div className="tv-list">
          {matches.map((match, i) => (
            <Row key={i} k={match.score !== null ? match.score.toFixed(3) : undefined}>
              ...
              {match.name && match.name !== match.label && (
                <span className="tv-faint"> {match.name}</span>
              )}
              {match.description && (
