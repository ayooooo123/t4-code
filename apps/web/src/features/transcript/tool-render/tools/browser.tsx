...
import { Badge, CodeBlock, ResultImages, ResultText, type Tone } from "../parts.tsx";
import type { ToolRenderer, ToolRenderProps } from "../types.ts";
import { detailsRecord, isRecord, num, shortenPath, str, truncate } from "../util.ts";
...
function Summary({ args, result }: ToolRenderProps): ReactNode {
  ...
  const closeAll =
    action === "close" && (args.all === true || (str(args.name) === null && details.name === null));
  ...
  return (
    <>
      ...
      {action === "run" && code !== null && (
        <CodeBlock code={code.replace(/\s+$/, "")} lang="javascript" />
      )}
