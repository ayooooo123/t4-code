...
import { detailsRecord, isRecord, normalizeWs, num, str, truncate } from "../util.ts";
...
function headerPath(line: string): string | null {
  ...
  if (body.length >= 2) {
    const first = body[0];
    if ((first === '"' || first === "'") && first === body[body.length - 1])
      body = body.slice(1, -1);
  }
...
function fileEntry(d: Record<string, unknown>): FileEntry {
  ...
  return {
    ...
    diagnostics: diag
      ? { summary: str(diag.summary), messages, errored: diag.errored === true }
      : null,
  };
}
  ...
  return (
    <div>
      ...
      {entry.isError
        ...
        : entry.diff !== null &&
          entry.diff.length > 0 && <DiffBlock diff={entry.diff} maxLines={40} />}
      {diag?.summary && <Note tone={diag.errored ? "err" : "warn"}>{diag.summary}</Note>}
      {diag !== null && diag.messages.length > 0 && (
        <Output text={diag.messages.join("\n")} maxLines={6} />
      )}
    </div>
  );
}
...
function Body({ args, result }: ToolRenderProps): ReactNode {
  ...
  const fallbackPath =
    str(args.file_path) ?? str(args.path) ?? (input ? (inputPaths(input)[0] ?? null) : null);
  ...
  return (
    <>
      ...
      {input !== null && input.length > 0 && (
        <Output text={input} variant="code" maxLines={10} title="input" />
      )}
      {input === null && (args.input !== undefined || args._input !== undefined) && (
