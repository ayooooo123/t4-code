...
import { Badge, Badges, InvalidArg, Output, ResultText, Row } from "../parts.tsx";
import type { ToolRenderer, ToolRenderProps } from "../types.ts";
import { normalizeWs, resultTextOf, str, truncate } from "../util.ts";
...
function Summary(props: ToolRenderProps): ReactNode {
  ...
  return (
    <>
      {query !== null ? (
        <span>{truncate(normalizeWs(query), 96)}</span>
      ) : (
      ...
      {found !== null && (
        <>
          {" "}
          {found > 0 ? (
            <Badge tone="accent">{found} found</Badge>
          ) : (
            <Badge tone="warn">no matches</Badge>
          )}
        </>
      )}
    </>
          ...
          <div className="tv-list">
            {entries.map((entry, i) => (
              <Row key={i}>
                ...
                {(entry.type !== null || entry.date !== null) && (
                  <Badges items={[entry.type, entry.date]} />
                )}
              </Row>
            ))}
          </div>
