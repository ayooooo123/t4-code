...
import { argsDigest, detailsRecord, isRecord, normalizeWs, num, str, truncate } from "../util.ts";
...
function renderCells(
  ...
  name: string,
  detailCells: DetailCell[],
): EvalCell[] {
  const cells = cellsFromArgs(args, name);
  if (cells.length > 0) return cells;
  return detailCells.map((c) => ({
    lang: c.lang ?? "py",
    title: c.title,
    attrs: [],
    code: c.code,
...
function Summary({ name, args, result }: ToolRenderProps): ReactNode {
  ...
  const first = cells[0]!;
  const label =
    first.title || normalizeWs(first.code.split("\n").find((l) => l.trim() !== "") ?? "");
...
function Body({ name, args, result }: ToolRenderProps): ReactNode {
  ...
  if (cells.length === 0) {
    const badArgs =
      !Array.isArray(args.cells) && str(args.input) === null && str(args.code) === null;
    return (
      <>
  ...
  return (
    <>
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <div className="tv-cell" key={`c${i}`}>
              <CodeBlock
                code={cell.code}
                lang={HLJS_LANG[cell.lang] ?? null}
                title={titleParts.join(" · ")}
              />
              {dc && dc.output !== "" && (
                <Output text={dc.output} maxLines={12} error={dc.status === "error"} />
              )}
            </div>
          );
        })}
      ...
      {jsonText && (
        <Output text={jsonText} lang="json" variant="code" maxLines={12} title="display" />
      )}
