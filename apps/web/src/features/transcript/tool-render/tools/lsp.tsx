...
import type { Tone } from "../parts.tsx";
import { Badge, InvalidArg, Kv, KvGrid, Output, PathText, ResultText, Row } from "../parts.tsx";
import type { ToolRenderer, ToolRenderProps } from "../types.ts";
import { detailsRecord, normalizeWs, num, resultTextOf, str, truncate } from "../util.ts";
...
function Body({ args, result }: ToolRenderProps): ReactNode {
  ...
  const locs =
    diags.length === 0 &&
    text &&
    ((action != null && LOCATION_ACTIONS[action]) || /\d+\s+reference\(s\)/.test(text))
      ? parseLocations(text)
      : [];
    ...
    <>
      <KvGrid>
        ...
        <ArgKv
          ...
          raw={args.file}
          val={
            file === "*" ? <Badge>workspace</Badge> : file && <PathText path={file} from={line} />
          }
        />
        ...
        <ArgKv
          k="new name"
          raw={args.new_name}
          val={newName && truncate(normalizeWs(newName), 120)}
        />
