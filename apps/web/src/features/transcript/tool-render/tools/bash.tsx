...
import { Badge, Badges, InvalidArg, ResultImages, ResultText, Row } from "../parts.tsx";
...
function Body({ args, result }: ToolRenderProps): ReactNode {
  ...
  if (wallTimeMs !== null) {
    stats.push(
      wallTimeMs < 1000
        ? `wall ${Math.round(wallTimeMs)}ms`
        : `wall ${(wallTimeMs / 1000).toFixed(1)}s`,
    );
  }
    ...
    <>
      ...
      <Badges
        items={[
          ...
          job && (
            <Badge
              tone={job.state === "failed" ? "err" : job.state === "running" ? "accent" : "ok"}
            >
              async {job.state}
            </Badge>
          ),
