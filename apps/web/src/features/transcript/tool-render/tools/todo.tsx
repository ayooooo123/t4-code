...
import { Badges, ResultText, Row } from "../parts.tsx";
    ...
    for (let t = 0; t < phase.tasks.length; t++) {
      ...
      rendered.push(
        <div key={`p${i}t${t}`} className={`tv-task tv-task--${status}`}>
          <span aria-hidden="true" className="tv-task-icon">
            {TASK_ICONS[status]}
          </span>
          <span className="sr-only">{TASK_STATUS_LABELS[status]}: </span>
          <span className="tv-task-content">{str(task.content) ?? ""}</span>
        </div>,
      );
    }
  }
