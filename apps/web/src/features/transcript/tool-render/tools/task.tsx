...
import { AgentLink, Badge, Note, Output, ResultText, Row } from "../parts.tsx";
import type { ToolRenderer, ToolRenderHost, ToolRenderProps } from "../types.ts";
  ...
  return (
    <>
      ...
      >
        ...
        {res.truncated === true && <Badge tone="warn">truncated</Badge>}{" "}
...
function Body({ args, result, host }: ToolRenderProps): ReactNode {
  ...
  const progress =
    details && Array.isArray(details.progress) ? details.progress.filter(isRecord) : [];
  ...
  const ordered = [...results].sort(
    (a, b) =>
      (num(a.durationMs) ?? 0) - (num(b.durationMs) ?? 0) ||
      (num(a.index) ?? 0) - (num(b.index) ?? 0),
  );
