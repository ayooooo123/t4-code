...
import { Badge, Badges, InvalidArg, Note, ResultText } from "../parts.tsx";
import type { ToolRenderer, ToolRenderProps } from "../types.ts";
import { detailsRecord, isRecord, num, scopePaths, shortenPath, str, truncate } from "../util.ts";

function Summary({ args }: ToolRenderProps): ReactNode {
  const raw = args.path ?? args.paths;
  if (raw !== undefined && typeof raw !== "string" && !Array.isArray(raw))
    return <InvalidArg what="path" />;
    ...
    <>
      <Badges
        items={[
          ...
          truncated && (
            <Badge tone="warn">
              {resultLimit !== null ? `truncated at ${resultLimit}` : "truncated"}
            </Badge>
          ),
        ]}
      />
      {missing.length > 0 && (
        <Note tone="warn">skipped missing: {missing.map(shortenPath).join(", ")}</Note>
      )}
