...
import { Badge, InvalidArg, Kv, KvGrid, Note, Output, ResultText, Row } from "../parts.tsx";
import type { ToolRenderer, ToolRenderProps } from "../types.ts";
import { detailsRecord, isRecord, normalizeWs, num, shortenPath, str, truncate } from "../util.ts";
...
/** A number, PR/issue URL, or branch name → a compact reference or truncated literal. */
...
function Summary(props: ToolRenderProps): ReactNode {
  ...
  return (
    <>
      {op ? <Badge tone="accent">{op}</Badge> : <Badge tone="warn">no op</Badge>}{" "}
...
function DetailsGrid({ details }: { details: Record<string, unknown> }): ReactNode {
  ...
  for (const key of DETAIL_KEYS) {
    ...
    if (typeof value === "number") {
      ...
      const text =
        key === "worktreePath" ? shortenPath(value) : key === "headSha" ? shortSha(value) : value;
      rows.push(
        <Kv k={key} key={key}>
          {text}
        </Kv>,
